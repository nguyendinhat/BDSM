...
import math
import mathutils
from .. context.items import normal_flatten_threshold_preset_items, loop_mapping_items, loop_mapping_dict
from .. utils.developer import output_traceback
from .. utils.graph import build_mesh_graph_normal
...
from .. utils.mesh import smooth, flip_normals, get_coords
from .. utils.registration import get_prefs
from .. utils.ui import init_cursor, wrap_cursor, draw_init, draw_title, draw_prop, update_HUD_location
from ....interface.hud import init_status, finish_status
from ....interface.draw import draw_mesh_wire
...
class BDSM_Mesh_Face_Normal_Flatten(bpy.types.Operator):
    bl_idname = "mesh.bdsm_mesh_face_normal_flatten"
    bl_label = "BDSM Mesh Face Normal Flatten"
    bl_description = "BDSM Mesh Face Normal Flatten \nFlatten uneven shading on (mostly) ngons"
...
class BDSM_Mesh_Face_Normal_Straighten(bpy.types.Operator):
    bl_idname = "mesh.bdsm_mesh_face_normal_straighten"
    bl_label = "BDSM Mesh Face Normal Straighten"
    bl_description = "BDSM Mesh Face Normal Straighten\nStraighten uneven shading on straight fuse surface sections"
    bl_options = {'REGISTER', 'UNDO'}
    ...
    def main(self, active, modal=False):
        ...
        for verts, edges, faces in islands:
            ...
            mg = build_mesh_graph_normal(bm, debug=debug)
...
class BDSM_Mesh_Face_Normal_Transfer(bpy.types.Operator):
    bl_idname = "mesh.bdsm_mesh_face_normal_transfer"
    bl_label = "BDSM Mesh Face Normal Transfer"
    bl_description = "BDSM Mesh Face Normal Transfer\nTransfer Normals from Stash"
    bl_options = {'REGISTER'}
...
class BDSM_Mesh_Face_Normal_Clear(bpy.types.Operator):
    bl_idname = "mesh.bdsm_mesh_face_normal_clear"
    bl_label = "BDSM Mesh Face Normal Clear"
    bl_description = "BDSM Mesh Face Normal Clear\nReset normals of the selected geometry, keep unselected geo as is"
    bl_options = {'REGISTER', 'UNDO'}
