...
def create_edges_item(context, bm, item):
    es = []
    loop = item.loop
...
def boolslice_surface(context, bm, source, other, loops, grid_len, propdepth, propoffset, bool_intersect):
    #delid = bm.faces.layers.int.new("gmdel")
    oldmode = bpy.context.tool_settings.mesh_select_mode
    ...
    for item in loops:
        ...
        bm.faces.index_update()
        bm.normal_update()
        fs = boolean_item(context, bm, f1, source, other, grid_len, propdepth, propoffset, 0)
        fsall += fs
        for f in bm.faces:
            f.select = False        
    ...
    if bool_intersect:
        if bpy.app.version >= (2, 91, 0):
            if use_exact:
                bpy.ops.mesh.intersect(mode='SELECT_UNSELECT', separate_mode='CUT', solver='EXACT')
            else:
