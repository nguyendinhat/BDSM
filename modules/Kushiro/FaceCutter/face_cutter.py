...
class BDSM_Mesh_Face_Cutter(Operator):
    """Tooltip"""
    bl_idname = "mesh.bdsm_mesh_face_cutter"
    bl_label = "BDSM Mesh Face Cutter"
    bl_options = {"REGISTER", "UNDO"}
    bl_description = """
        BDSM Mesh Face Cutter - Created by Kushiro
        is a handy modeling tool designed for cutting ngon face easily.
        It allows you to cut perpendicular lines (or rotated) on selected face, by easily defined snapping points.
    """
    ...
    def handle_cut(self, context, event):
        ...
        elif event.type == {'Q','RET','NUMPAD_ENTER'}:
            if event.value == 'PRESS':
    ...
    def handle_linked(self, context, event):
        ...
        if event.type == {'Q','RET','NUMPAD_ENTER'}:
            if event.value == 'PRESS':
                self.cut_face(context)
                self.cleanup()
                gui.draw_handle_remove()
    ...
    def print_hint(self):
        ...
        if last == self.overall:
            ...
            txt = ['(Ctrl + mouse scroll) Number of cuts : ' + str(self.snaps),
                'Holding Shift: turn off snapping',
                'A: Align to closest edge',
                'Z / X / C / V : Line rotation',
                'G: Fill lines |', 
                'D: 45 degree',
                'F: 30 degree',
                'S: Symmetric copy | Ctrl + Z : Undo',
                'Q: Confirm',
                'R: Cut through (Shift R : limited)',
                'T: Cut through and Break apart',
                '1 - 9 keys : Set cuts',
                'Press Esc to exit.']
        elif last == self.handle_linked:
            txt = ['E: Exit Easy Edit mode',
                '(Ctrl + mouse scroll) Number of cuts',
                'Ctrl + Z : undo',
                'Q: Confirm',
                'R: Cut through',
                '1 - 9 keys : Set cuts',
