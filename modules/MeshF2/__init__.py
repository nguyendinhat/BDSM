...
def quad_from_edge(bm, edge_sel, context, event):
    ...
    if addon_prefs.f2_quad_from_e_mat:
...
def quad_from_vertex(bm, vert_sel, context, event):
    ...
    if addon_prefs.f2_quad_from_v_mat:
        mat_index = bpy.context.object.active_material_index
...
def expand_vert(self, context, event):
    ...
    if addon_prefs.f2_tris_from_v_mat:
...
class BDSM_Mesh_F2(bpy.types.Operator):
    bl_idname = 'mesh.bdsm_mesh_f2'
    bl_label = 'BDSM Mesh F2'
    bl_description = 'BDSM Mesh F2/n'\
...
# registration
def register():
    bpy.utils.register_class(BDSM_Mesh_F2)

def unregister():
    bpy.utils.unregister_class(BDSM_Mesh_F2)