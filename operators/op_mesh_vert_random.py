...
from bpy.props import  BoolProperty, FloatProperty, IntVectorProperty
...
def add_object(self, context, valmin, valmax, factor, vgfilter):
    ...
    if warn_message:
        self.report({'WARNING'},
                    'Some of the Selected Vertices dont have a Group with Vertex Weight assigned')
    bpy.ops.object.mode_set(mode=mode)


class BDSM_Mesh_Vert_Random(Operator):
    bl_idname = 'mesh.bdsm_mesh_vert_random'
    bl_label = 'BDSM Mesh Verts Random'
    bl_description = ('BDSM Mesh Verts Random \nRandomize the location of vertices by a specified\n'
