...
class BDSM_Mesh_Mirror_Flip(Operator):
    bl_idname = 'mesh.bdsm_mesh_mirror_flip'
    bl_label = 'BDSM Mesh Mirror / Flip'
    bl_description = 'BDSM Mesh Mirror / Flip'
    ...
    def execute(self, context):
        ...
        og = tools.getset_transform(setglobal=False)
        self.ot = og[0]
        ...
        if og[0] == 'GLOBAL':
            if not em:
                ...
                avg_pos = tools.average_vector(bbcoords)
            ...
            bbmin, bbmax = tools.bbox_minmax(bbcoords)
            ...
            if self.mode == 'FLIP' and og[0] == 'LOCAL':
                ...
                bbmin, bbmax = [], []
                if em:
                    avg_pos = tools.average_vector([Vector(i) for i in bbcoords])
                else:
                    avg_pos = active_obj.matrix_world @ tools.average_vector([Vector(i) for i in bbcoords])
        ...
        #
            ...
            avg_pos = active_obj.matrix_world @ (tools.average_vector([v.co for v in active_verts]))
